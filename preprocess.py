...
def build_save_in_shards_using_shards_size(src_corpus, tgt_corpus, src_ref_corpus, tgt_ref_corpus, fields,
                                           corpus_type, opt):
    """
    ...
    with codecs.open(src_corpus, "r", encoding="utf-8") as fsrc:
        with codecs.open(tgt_corpus, "r", encoding="utf-8") as ftgt:
            with codecs.open(src_ref_corpus, "r", encoding="utf-8") as frsrc:
                with codecs.open(tgt_ref_corpus, "r", encoding="utf-8") as frtgt:
                    ...
                    logger.info("Reading reference source and target files: %s %s."
                                % (src_ref_corpus, tgt_ref_corpus))
                    src_ref_data = frsrc.readlines()
                    tgt_ref_data = frtgt.readlines()
                    if len(src_ref_data) != len(tgt_ref_data):
                        raise AssertionError("Source and Target should \
                                            have the same length")

                    num_shards = int(len(src_ref_data) / opt.shard_size)
                    for x in range(num_shards):
                        logger.info("Splitting shard %d." % x)
                        f = codecs.open(src_ref_corpus + ".{0}.txt".format(x), "w",
                                        encoding="utf-8")
                        f.writelines(
                                src_ref_data[x * opt.shard_size: (x + 1) * opt.shard_size])
                        f.close()
                        f = codecs.open(tgt_ref_corpus + ".{0}.txt".format(x), "w",
                                        encoding="utf-8")
                        f.writelines(
                                tgt_ref_data[x * opt.shard_size: (x + 1) * opt.shard_size])
                        f.close()
                    num_written = num_shards * opt.shard_size
                    if len(src_ref_data) > num_written:
                        logger.info("Splitting shard %d." % num_shards)
                        f = codecs.open(src_ref_corpus + ".{0}.txt".format(num_shards),
                                        'w', encoding="utf-8")
                        f.writelines(
                                src_ref_data[num_shards * opt.shard_size:])
                        f.close()
                        f = codecs.open(tgt_ref_corpus + ".{0}.txt".format(num_shards),
                                        'w', encoding="utf-8")
                        f.writelines(
                                tgt_ref_data[num_shards * opt.shard_size:])
                        f.close()

    src_list = sorted(glob.glob(src_corpus + '.*.txt'))
    tgt_list = sorted(glob.glob(tgt_corpus + '.*.txt'))

    src_ref_list = sorted(glob.glob(src_ref_corpus + '.*.txt'))
    tgt_ref_list = sorted(glob.glob(tgt_ref_corpus + '.*.txt'))
    ...
    for index, src in enumerate(src_list):
        ...
        dataset = inputters.build_dataset(
            ...
            src_ref_path=src_ref_list[index],
            tgt_ref_path=tgt_ref_list[index],
            src_dir=opt.src_dir,
