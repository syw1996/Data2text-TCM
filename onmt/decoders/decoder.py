...
class RNNDecoderBase(nn.Module):
    ...
    def forward(self, tgt, memory_bank, tgt_memory_bank, memory_lengths=None,
                step=None):
        """
        Args:
        ...
        dec_state, dec_outs, attns = self._run_forward_pass(
            tgt, memory_bank, tgt_memory_bank, memory_lengths=memory_lengths)
...
class InputFeedRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, tgt_memory_bank, memory_lengths=None):
        """
        ...
        emb = self.embeddings(tgt)
        assert emb.dim() == 3  # len x batch x emb_dim

        dec_state = self.state["hidden"]
        coverage = []
        coverage.append(self.state["coverage"].squeeze(0) \
        ...
        for i, emb_t in enumerate(emb.split(1)):
            ...
            if self._coverage:
                coverage.append(coverage[i] + p_attn \
                    if coverage[i] is not None else p_attn)
                '''
                coverage = coverage + p_attn \
                    if coverage is not None else p_attn
                '''
                attns["coverage"] += [coverage[i+1]]
