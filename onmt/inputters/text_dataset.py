...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, src_ref_examples_iter, tgt_ref_examples_iter,
        ...
        if tgt_examples_iter is not None:
            examples_iter = (self._join_dicts(src, tgt, ref_src, ref_tgt) for src, tgt, ref_src, ref_tgt in
                             zip(src_examples_iter, tgt_examples_iter, src_ref_examples_iter, tgt_ref_examples_iter))
        else:
    ...
    @staticmethod
    def collapse_copy_ref_scores(scores,
                                 batch,
                                 tgt_vocab,
                                 src_vocabs,
                                 batch_dim=1,
        ...
        for b in range(scores.size(batch_dim)):
            ...
            for i in range(1, len(src_vocab)):
                ...
                if ti != 0:
                    ...
                    fill.append(ti)
            if blank:
                blank = torch.Tensor(blank).type_as(batch.ref_indices.data)
                fill = torch.Tensor(fill).type_as(batch.ref_indices.data)
                score = scores[:, b] if batch_dim == 1 else scores[b]
                new_score = score.index_select(1, blank)
                score.index_add_(1, fill, new_score)
                score.index_fill_(1, blank, 1e-10)
        return scores
    ...
    def make_examples(text_iter, truncate, side):
        ...
        for i, line in enumerate(text_iter):
            ...
            if 'ref' not in side:
                ...
                example_dict = {side: words, "ref_indices": i}
            if feats:
                prefix = side + "_feat_"
