...
def get_num_features(data_type, corpus_file, side):
    ...
    """
    assert side in ["src", "tgt", "ref_src", "ref_tgt"]
...
def make_features(batch, side, data_type='text'):
    ...
    if 'ref' not in side:
        keys = sorted([k for k in batch.__dict__ if feat_start in k and 'ref' not in k])
    else:
...
def build_dataset(fields, data_type, src_data_iter=None, src_ref_data_iter=None, src_path=None,
                  src_dir=None, tgt_data_iter=None, tgt_path=None, src_ref_path=None, tgt_ref_data_iter=None, tgt_ref_path=None,
    ...
    def _make_examples_nfeats_tpl(data_type, src_data_iter, src_path, src_dir,
        ...
        if data_type == 'text':
            ...
            src_ref_examples_iter, _ = \
                TextDataset.make_text_examples_nfeats_tpl(
                    src_ref_data_iter, src_ref_path, src_seq_length_trunc, "ref_src")
        ...
        return src_examples_iter, src_ref_examples_iter, num_src_feats

    src_examples_iter, src_ref_examples_iter, num_src_feats = \
    ...
    tgt_ref_examples_iter, _ = \
        TextDataset.make_text_examples_nfeats_tpl(
            tgt_ref_data_iter, tgt_ref_path, tgt_seq_length_trunc, "ref_tgt")

    if data_type == 'text':
        dataset = TextDataset(fields, src_examples_iter, tgt_examples_iter, src_ref_examples_iter, tgt_ref_examples_iter,
...
def lazily_load_dataset(corpus_type, data_name):
    ...
    pts = sorted(glob.glob(data_name + '.' + corpus_type + '.[0-9]*.pt'))
    if pts:
        for pt in pts:
            yield _lazy_dataset_loader(pt, corpus_type)
    else:
        # Only one inputters.*Dataset, simple!
        pt = data_name + '.' + corpus_type + '.pt'
        yield _lazy_dataset_loader(pt, corpus_type)
