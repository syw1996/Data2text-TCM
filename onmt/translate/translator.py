...
class Translator(object):
    ...
    def translate(self,
        ...
        data = inputters. \
            build_dataset(self.fields,
                          ...
                          src_ref_path=src_path,
                          tgt_ref_path=ref_path,
                          src_dir=src_dir,
    ...
    def _run_encoder(self, batch, data_type):
        ...
        ref = inputters.make_features(batch, 'ref_tgt', data_type)
        src_lengths = None
        ref_lengths = None
        if data_type == 'text':
        ...
        # source encoder
        enc_states, memory_bank, src_lengths = self.model.src_encoder(
            src, src_lengths)
        # reference encoder
        ref_enc_states, ref_memory_bank, ref_lengths = self.model.tgt_encoder(
            ref, ref_lengths)
        
        # interactive attention
        if self.model.co_attention is not None:
            ref_memory_bank = self.model.co_attention(enc_states, memory_bank, ref_memory_bank, src_lengths, ref_lengths)
        ...
        if ref_lengths is None:
            assert not isinstance(ref_memory_bank, tuple), \
                'Ensemble decoding only supported for text data'
            ref_lengths = torch.Tensor(batch.batch_size) \
                               .type_as(ref_memory_bank) \
                               .long() \
                               .fill_(ref_memory_bank.size(0))
        return src, enc_states, memory_bank, src_lengths, ref, ref_enc_states, ref_memory_bank, ref_lengths

    def _decode_and_generate(self, decoder_input, memory_bank, ref_memory_bank, batch, data,
    ...
    def _fast_translate_batch(self,
        ...
        # Encoder forward.
        src, enc_states, memory_bank, src_lengths, _, _, ref_memory_bank, _ = self._run_encoder(
        ...
        if "tgt" in batch.__dict__:
            results["gold_score"] = self._score_target(
                batch, memory_bank, ref_memory_bank, src_lengths, data, batch.src_map
        ...
        if isinstance(ref_memory_bank, tuple):
            ref_memory_bank = tuple(tile(x, beam_size, dim=1) for x in ref_memory_bank)
            mb_device = ref_memory_bank[0].device
        else:
            ref_memory_bank = tile(ref_memory_bank, beam_size, dim=1)
            mb_device = memory_bank.device
        ...
        for step in range(max_length):
            ...
            log_probs, attn = \
                self._decode_and_generate(decoder_input, memory_bank, ref_memory_bank,
                                          batch, data,
                                          memory_lengths=memory_lengths,
                                          src_map=src_map,
                                          step=step,
    ...
    def _translate_batch(self, batch, data):
        ...
        for i in range(self.max_length):
            ...
            out, beam_attn = \
                self._decode_and_generate(inp, memory_bank, ref_memory_bank, batch, data,
                                          memory_lengths=memory_lengths,
    ...
    def _score_target(self, batch, memory_bank, ref_memory_bank, src_lengths, data, src_map):
        tgt_in = inputters.make_features(batch, 'tgt')[:-1]

        log_probs, attn = \
            self._decode_and_generate(tgt_in, memory_bank, ref_memory_bank, batch, data,
